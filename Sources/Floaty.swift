...
extension String {
    ...
    private func localized(_ lang: String) -> String {
...
open class Floaty: UIView {
     ...
     */
    @objc open var paddingX: CGFloat = 10 {
        didSet {
            self.setNeedsDisplay()
        }
    }
     ...
     */
    @IBInspectable
    @objc open var buttonColor: UIColor = .clear
     ...
     */
    @IBInspectable
    @objc open var plusColor: UIColor = UIColor(red: 1, green: 1, blue: 1, alpha: 1)
    
    /**
     Background overlaying color.
     */
    @IBInspectable
    @objc open var overlayColor: UIColor = UIColor(red: 0, green: 0, blue: 0, alpha: 0.7)
    
    /**
     The space between the item and item.
     */
    @IBInspectable
    @objc open var itemSpace: CGFloat = 10
    ...
    @objc open var isCanMakeOn : Bool = true
    @objc open var isEndProject : Bool = false
    
    @objc open var isExistItem : Bool = true
    
    @objc open var isTabbarOn : Bool = true
    ...
    private var colors: [CGColor]? = nil
    // MARK: - Initialize
    ...
    @objc open func setGradient() {
        print("changmin log - called floaty origin setGradient: \(colors)")
        ...
        gradient.colors = colors ?? [
            UIColor(red: 0.933, green: 0.144, blue: 0.854, alpha: 1).cgColor,
            UIColor(red: 0.348, green: 0.14, blue: 0.988, alpha: 1).cgColor
        ]
        
        circleLayer.addSublayer(gradient)
    }
    
    @objc open func setGradient(_ color1: UIColor, _ color2: UIColor) {
        print("changmin log - called floaty setGradient")
        let gradient = CAGradientLayer()
        gradient.frame = circleLayer.frame
    ...
    open override func draw(_ rect: CGRect) {
        ...
        setCircleLayer()
        print("changmin log - called floaty draw")
        setGradient()
        if buttonImage == nil {
            setPlusLayer()
        } else {
    ...
    @objc public func open() {
        ...
        if(isCanMakeOn && !isEndProject){
            ...
            if(items.count > 0){
                ...
                let infoTitleStr = "PRJDETAIL_I_067".localized
                let guideViewSize = (infoTitleStr as NSString).size(withAttributes: [NSAttributedString.Key.font:UIFont.boldSystemFont(ofSize: 12.0)])
                
                let guideView : UIView = UIView(frame: CGRect(x: UIScreen.main.bounds.size.width - (guideViewSize.width+(10*2)) - 10, y: (UIScreen.main.bounds.size.height - size - 10) - paddingY - 60, width: guideViewSize.width+(10*2), height: 60))
                guideView.backgroundColor = UIColor.white
                guideView.layer.cornerRadius = 6.0
                
                let label1 : UILabel = UILabel(frame:CGRect(x: 0,
                                                            y: isEndProject ? 0 : 13,
                                                            width: guideViewSize.width+(10*2),
                                                            height:  isEndProject ? 60 : 13))
                label1.font = UIFont.boldSystemFont(ofSize: 12.0)
                label1.text = isEndProject ? "PRJEND_I_008".localized : "PRJDETAIL_I_067".localized
                label1.textAlignment = .center
                label1.numberOfLines = 0
                label1.textColor = UIColor.init(red: 17/255, green: 17/255, blue: 17/255, alpha: 1.0)
                guideView.addSubview(label1)
                
                if (!isEndProject) {
                    let label2 : UILabel = UILabel(frame:CGRect(x: 0, y: 60 - 12-16, width: guideViewSize.width+(10*2), height: 12))
                    label2.font = UIFont.systemFont(ofSize: 10)
                    label2.text = "PRJDETAIL_I_068".localized
                    label2.textAlignment = .center
                    label2.textColor = UIColor.init(red: 17/255, green: 17/255, blue: 17/255, alpha: 1.0)
                    guideView.addSubview(label2)
                }
    ...
    fileprivate func setButtonImage() {
        ...
        buttonImageView.frame = circleLayer.frame
        buttonImageView = UIImageView(image: buttonImage)
//        buttonImageView.contentMode = .top
    ...
    fileprivate func plusBezierPath() -> UIBezierPath {
        ...
        path.move(to: CGPoint(x: size/2, y: size/2-7))
        path.addLine(to: CGPoint(x: size/2, y: size-(size/2-7)))
        path.move(to: CGPoint(x: size/2-7, y: size/2))
        path.addLine(to: CGPoint(x: size-(size/2-7), y: size/2))
        return path
    }
    ...
    fileprivate func setRightBottomFrame(_ keyboardSize: CGFloat = 0) {
        ...
        if superview == nil {
            frame = CGRect(
                x: (UIScreen.main.bounds.size.width - size) - paddingX , //-55는 bottom tab 높이
                y: (UIScreen.main.bounds.size.height - size - keyboardSize) - paddingY ,
                width: size,
                height: size
            )
...
//        if friendlyTap == true {
//            frame.size.width += 14 //paddingX
//            frame.size.height += 14 // paddingY
//        }
    }
    ...
    @objc internal func keyboardWillShow(_ notification: Notification) {
        ...
        var y_val: CGFloat
        if keyboardSize == 0 {
            y_val = UIScreen.main.bounds.height - self.size - self.paddingY
            
        } else {
            if self.paddingY >= 65 {
                y_val = UIScreen.main.bounds.height - self.size - self.paddingY - keyboardSize + 10
            } else {
                y_val = UIScreen.main.bounds.height - self.size - self.paddingY - keyboardSize
            }
        }
